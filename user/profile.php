...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_address'])) {
 ...
 if (!empty($new_address)) {
    $addr = $conn->real_escape_string($new_address);
    $conn->query("INSERT INTO user_addresses (user_id, address) VALUES ($user_id, '$addr')");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_address'])) {
    $updated_address = trim($_POST['updated_address']);

    if (!empty($updated_address)) {
        $addr = $conn->real_escape_string($updated_address);
...
// --- FETCH ADDRESSES ---
$addresses = $conn->query("SELECT address FROM users WHERE id = $user_id");
?>
    ...
    <?php echo $message; ?>
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                      ...
                      <a href="#" class="option-tile" data-bs-toggle="modal" data-bs-target="#addressModal">
    ...
    <div class="modal-dialog">
        <div class="modal-content shadow-lg rounded-4">
            ...
            <div class="modal-body">
                ...
                <!-- SHOW ADDRESSES -->
<?php 
$row = $addresses->fetch_assoc();
if(!empty($row['address'])): 
?>
<div class="border p-3 rounded mb-2">
    <?php echo htmlspecialchars($row['address']); ?>
    
    <button class="btn btn-sm btn-outline-primary mt-2" onclick="enableEdit()">Edit</button>
</div>

<!-- EDIT FORM -->
<form method="POST" id="editAddressForm" style="display:none;">
    <input type="hidden" name="update_address" value="1">
    <textarea name="updated_address" class="form-control mb-2"><?php echo htmlspecialchars($row['address']); ?></textarea>
    <button class="btn btn-success btn-sm">Update Address</button>
</form>
<?php else: ?>
...
<?php endif; ?>
                ...
                <form method="POST">
                    ...
                    <label class="fw-bold small mb-2">Add New Address</label>
                    <textarea name="new_address" class="form-control mb-3"></textarea>
